...
public abstract class GeneralRegion implements GeneralRegionInterface, Comparable<GeneralRegion>, ReplacementProvider {
     ...
     * @return
     */
    private long calculateVolumeExact(ProtectedRegion region, Vector min, Vector max) {
        long surface = 0;
    ...
    /**
     * Estimate, but fast algorithm
     * @param region
     * @return
     */
    private long calculateVolumeEstimate(ProtectedRegion region) {
    ...
    private long calculateVolume() {
        ...
        if (getWidth() * getDepth() < 100) {
            return calculateVolumeExact(region,min,max);
        }
        // Estimate, but quick algorithm
        return calculateVolumeEstimate(region);
    }
}
